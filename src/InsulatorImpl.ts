...
export class InsulatorImpl implements Insulator {
  ...
  constructor(
    ...
    (broadcastInterval > 0
      ? observable.pipe(bufferWhen(() => interval(broadcastInterval))).pipe(flatMap(x => pipe.send(x)))
      : observable.pipe(flatMap(x => pipe.send([x])))
    ).subscribe(x => callback(x, true), e => callback(e, false), () => exitCallback());
  }
